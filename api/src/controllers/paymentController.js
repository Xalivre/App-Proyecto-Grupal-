...
export const getPaymentHistoryUser = async (req, res) => {
  //admin
    ...
    if (email) {
      ...
      if (userDB.paymentHistory.length > 0) {
        ...
        if (payment.length === 0)
          return res
            .status(404)
            .send({ message: 'Payment not found or non-existent ID' });
...
export const getPaymentHistoryById = async (req, res) => {
  ...
  try {
    ...
    userDB
      ? res.status(200).json(userDB.paymentHistory)
...
export const getPaymentById = async (req, res) => {
  try {
    const paymentDB = await Payment.findById(req.params.id);
    paymentDB ? res.json(paymentDB) : res.status(404).send('Payment not found');
  } catch (e) {
    return res.json({ msg: `Error 404 - ${e}` });
  }
};
